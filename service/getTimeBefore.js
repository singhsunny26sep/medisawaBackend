...
exports.canBookAppointment = (appointmentDate, appointmentTime, doctor) => {
  ...
  const doctorEndTime = doctor.endTime
    ? moment(`${appointmentDate} ${doctor.endTime}`, "YYYY-MM-DD HH:mm")
    : null;
  const bookingBeforeTime = doctor.bookingBeforeTime || 0; // Default to 0 if null
  // Calculate the latest possible booking time
  const latestBookingTime = doctorEndTime
    ? doctorEndTime.clone().subtract(bookingBeforeTime, "hours")
    : null;
