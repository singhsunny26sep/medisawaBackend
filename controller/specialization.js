...
exports.getSpecialization = async (req, res) => {
    ...
    try {
        if (id) {
            ...
            return res.status(404).json({ success: false, msg: "Specialization not found!" })
        }
...
exports.getAllSpecializationPagination = async (req, res) => {
    try {
        ...
        return res.status(404).json({ success: false, msg: "No specialization found!" });
    } catch (error) {
...
exports.addSpecialization = async (req, res) => {
    ...
    try {
        ...
        if (checkDepartments) {
            return res.status(400).json({ success: false, msg: "Specialization already exists!" })
        }
        let imageUrl = await uploadToCloudinary(image.tempFilePath)
        const newDepartment = new Specialization({ name, description, image: imageUrl })
        const result = await newDepartment.save()
        if (result) {
            return res.status(201).json({ success: true, msg: "Specialization added successfully!", data: result })
        }
        return res.status(400).json({ success: false, msg: "Failed to add specialization!" })
    } catch (error) {
...
exports.updateSpecialization = async (req, res) => {
    ...
    try {
        ...
        if (result) {
            return res.status(200).json({ success: true, msg: "Specialization updated successfully!", data: result })
        }
        return res.status(400).json({ success: false, msg: "Failed to update specialization!" })
    } catch (error) {
...
exports.deleteSpecialization = async (req, res) => {
    ...
    try {
        ...
        if (result) {
            return res.status(200).json({ success: true, msg: "Specialization deleted successfully!" })
        }
        return res.status(400).json({ success: false, msg: "Failed to delete specialization!" })
    } catch (error) {
