...
exports.getAllDoctors = async (req, res) => {
    ...
    try {
        if (id) {
            ...
            const cart = await Cart.findOne({ userId: userId, doctorId: result?.doctorId?._id });
            if (result) {
                return res.status(200).json({ success: true, result, cart })
            }
            return res.status(404).json({ success: false, msg: "Doctor not found!" })
        }
        const result = await User.find({ role: 'doctor' }).select("-password -__v -patientId").sort({ createdAt: -1 }).populate({
            path: 'doctorId', // Populate the doctorId field
            populate: [
                { path: 'department', select: 'name' }, // Populate the department field inside doctor
                { path: 'specialization', select: 'name' } // Populate the specialization field inside doctor
...
exports.topRatedDoctor = async (req, res) => {
    ...
    try {
        ...
        return res.status(200).json({ msg: "Ok", success: true, result: result[0] });
...
exports.getDoctorProfile = async (req, res) => {
    ...
    try {
        const doctor = await Doctor.findOne({ userId: id }).populate({ path: "specialization", select: "name _id" }).populate({ path: "department", select: "name _id" })
        if (doctor) {
...
exports.searchDoctorByCategoryOrbySpacailization = async (req, res) => {
    ...
    try {
        ...
        const result = await Doctor.find(query).populate({ path: "specialization", select: "name" }).populate({ path: "department", select: "name" })
        if (result) {
            return res.status(200).json({ success: true, result })
        }
        return res.status(404).json({ success: false, msg: 'No doctors found!' })
...
exports.searchBySymptom = async (req, res) => {
    ...
    try {
        ...
        const doctors = await Doctor.find({ symptom: { $in: symptomsArray } }).populate({ path: "specialization", select: "name" }).populate({ path: "department", select: "name" })

        return res.status(200).json({ success: true, count: doctors.length, result: doctors });
...
exports.addProfile = async (req, res) => {
    ...
    try {
        ...
        if (checkUser.role !== 'doctor') {
            return res.status(403).json({ msg: 'You are not authorized to create a doctor profile!', success: false })
        }
        if (!checkUser) {
        ...
        if (!checkDoctory) {
            ...
            const newDoctor = new Doctor({ userId: id, name: name ? name : checkUser?.name, specialization, experience, department, image: imageUrl ? imageUrl : checkUser?.image, startTime, endTime, clinicAddress, contactNumber, clinicContactNumber, coordinates, dob, gender, email: email ? email : checkUser?.email, fee, oldFee, bookingBeforeTime, address })
            ...
            if (result) {
                return res.status(200).json({ msg: 'Profilie completed successfully', success: true, result })
            }
        }
        ...
        if (clinicContactNumber) checkDoctory.clinicContactNumber = clinicContactNumber
        ...
        // let result = "ok"
        if (result) {
            return res.status(200).json({ msg: `Doctor ${name ? name : checkDoctory?.name} profile updated successfully.`, success: true, result })
        }
        return res.status(400).json({ msg: `Failed to updated doctor ${name ? name : checkDoctory.name} profile!`, success: false })
...
exports.addReceptOfPatient = async (req, res) => {
    ...
    try {
        ...
        if (!checkDoctor) {
            return res.status(403).json({ error: "You are not authorized to add a recept" });
        }
        ...
        const newPrescription = new Prescription({ patientId, doctorId: checkDoctor?.doctorId, appointmentId, medicines, prescription, clinicSummary, allergy, details, invastigationAdvice, bp, pr, temp, spo, rr, rbs, clinicIssues });
...
exports.addLabTest = async (req, res) => {
    ...
    try {
        ...
        const newLabTest = new LabTest({ patientId, doctorId: checkDoctor?.doctorId, testName: checkLab.name, test: testId, labId: checkLab?.userId, price: checkLab.price, appointmentId });
        // Save the lab test to the database
        await newLabTest.save();
        return res.status(201).json({ message: "Lab test added successfully", labTest: newLabTest });
    } catch (error) {
