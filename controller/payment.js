...
exports.placeOrder = async (req, res) => {
    ...
    try {
        ...
        instance.orders.create(options, async (err, order) => {
            ...
            const result = await Transaction.create({ userId, amount, type, orderId: order.id, payment: (order.amount / 100) })
...
exports.verifyPayment = async (req, res) => {
    ...
    try {
        ...
        if (type == 'package') {
            ...
            if (!checkPackage) {
                return res.status(404).json({ msg: 'Package not found!', success: false })
            }
            checkPackage?.tests?.map(async (item) => {
                await LabTest.findOneAndUpdate({ test: item, patientId: checkUser?.patientId[0] }, { paid: true, })
            })
        }
        ...
        if (result) {
            ...
            return res.status(200).json({ msg: 'Payment done successfully.', success: true })
        }
        return res.status(401).json({ msg: 'Faild to proceed payment!', success: false })
    } catch (error) {
