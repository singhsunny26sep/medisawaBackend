...
exports.getMyCart = async (req, res) => {
  ...
  try {
    const cartItems = await MedicineCart.find({ userId })
      .sort({ createdAt: -1 })
      .populate("userId", "-password -__v -role -fcmToken -patientId")
      .populate({
        path: "medicineId",
        populate: [
    ...
    if (cartItems.length === 0) {
      return res
        .status(200)
        .json({ success: true, msg: "Cart is empty!", data: [] });
    }
...
exports.addToMedicineCart = async (req, res) => {
  ...
  try {
    ...
    const existingCartItem = await MedicineCart.findOne({
      userId,
      medicineId: id,
    });
    if (existingCartItem) {
      ...
      return res.status(200).json({
        success: true,
        msg: "Medicine quantity updated in cart!",
        result: existingCartItem,
      });
    }

    const newCartItem = await MedicineCart.create({
      userId,
      medicineId: id,
      quantity,
    });
...
exports.removeFromMedicineCart = async (req, res) => {
  ...
  try {
    const deletedItem = await MedicineCart.findOneAndDelete({
      userId,
      medicineId: id,
    });
    ...
    if (!deletedItem) {
      return res
        .status(404)
        .json({ success: false, msg: "Medicine not found in cart!" });
    }

    return res
      .status(200)
      .json({ success: true, msg: "Medicine removed from cart!" });
  } catch (error) {
...
exports.updateMedicineCart = async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json({
      success: true,
      msg: "Medicine quantity updated successfully!",
      data: cartItem,
    });
  } catch (error) {
