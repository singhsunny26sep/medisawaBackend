...
exports.getAllBooking = async (req, res) => {
  ...
  try {
    const result = await Booking.find({ receptionistId: userId })
      .sort({ createdAt: -1 })
      .populate("patientId")
      .populate("userId")
      .populate("doctorId");
...
exports.testPayment = async (req, res) => {
  ...
  try {
    if (!labTestId) {
      return res
        .status(400)
        .json({ success: false, msg: "Lab Test ID is required" });
    }
    ...
    if (paidAmountVal <= 0) {
      return res
        .status(400)
        .json({ success: false, msg: "Paid amount must be greater than zero" });
    }
    ...
    const totalPaid = labTest.paidAmounts.reduce(
    ...
    if (newTotalPaid > labTest.price) {
      return res
        .status(400)
        .json({ success: false, msg: "Payment exceeds total price." });
    }
...
exports.paymentMultipleLabTest = async (req, res) => {
  ...
  try {
    if (!labTestIds || !Array.isArray(labTestIds) || labTestIds.length === 0) {
      return res
        .status(400)
        .json({ success: false, msg: "Lab Test IDs are required" });
    }
    ...
    if (results.length === 0) {
      return res
        ...
        .json({
          success: false,
          msg: "No applicable lab tests to apply payment.",
        });
    }
    ...
    return res
      ...
      .json({
        success: true,
        msg: "Payment distributed among lab tests successfully",
        result: results,
  ...
  try {
    ...
    if (checkVital) {
      ...
      return res
        ...
        .json({
          success: true,
          msg: "Vital record updated successfully",
          data: checkVital,
        });
    }
    ...
    return res
      ...
      .json({
        success: true,
        msg: "Vital record added successfully",
        data: vital,
      });
  } catch (error) {
