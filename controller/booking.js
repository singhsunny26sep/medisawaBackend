...
const { sendSingleNotification, bookingNotification, bookingCancelNotification } = require('../service/notification');
...
exports.getAllBookings = async (req, res) => {
    ...
    try {
        if (id) {
            const result = await Booking.findById(id).populate("patientId").populate("userId").populate("doctorId")
            if (result) {
                const prescription = await Prescription.find({ appointmentId: id }).sort({ createdAt: -1 })
                const labTest = await LabTest.find({ appointmentId: id }).populate("reports")
                const report = await Report.find({ appointmentId: id })

                return res.status(200).json({ success: true, result, prescription, labTest, report });
            }
            return res.status(404).json({ success: false, msg: "Booking not found!" });
        }
        const result = await Booking.find().populate("patientId").populate("userId").populate("doctorId")
...
exports.getBookingByPatientId = async (req, res) => {
    ...
    try {
        ...
        const result = await Booking.find(query).sort({ createdAt: -1 }).populate({ path: "userId", select: "-password -role -patientId -__v -updatedAt" }).populate({ path: "doctorId", select: "-password -role -__v -updatedAt -dob -createdAt" }).populate({ path: "patientId" }).lean();
...
exports.getDoctorByIdBooking = async (req, res) => {
    ...
    try {
        const result = await Booking.find({ doctorId: id }).populate("userId").populate("patientId").populate("doctorId")
        if (result) {
            return res.status(200).json({ success: true, result })
        }
        return res.status(404).json({ success: false, msg: "No bookings found!" });
...
exports.getBookingDoctor = async (req, res) => {
    ...
    try {
        ...
        const todayBookings = await Booking.find({ doctorId: checkDoctor?._id, appointmentDate: currentDate }).sort({ createdAt: -1 }).populate("userId").populate("patientId").populate("doctorId");
        ...
        if (todayBookings.length > 0) {
            ...
            return res.json({
                ...
                currentPage: page,
                totalPages: Math.ceil((todayBookings.length + remainingBookings.length) / limit),
        ...
        const totalRecords = await Booking.countDocuments({ doctorId: checkDoctor?._id });

        return res.json({ result: allBookings, currentPage: page, totalPages: Math.ceil(totalRecords / limit), totalRecords });
        // return res.status(404).json({ success: false, msg: "No bookings found!" });
    } catch (error) {
...
exports.getBookingData = async (req, res) => {
    try {
        ...
        if (!checkDoctorUser?.doctorId) {
            return res.status(404).json({ success: false, message: 'Doctor not linked' });
        }
        ...
        if (!startTime || !endTime) {
            return res.status(400).json({ success: false, message: 'Doctor working hours not set' });
        }
        ...
        const bookedTimes = bookedAppointments.map(booking => booking.appointmentTime);


        const slotData = slots.map(slot => ({ time: slot, isBooked: bookedTimes.includes(slot) }));

        return res.status(200).json({ success: true, slots: slotData, result: doctor });
...
exports.addBooking = async (req, res) => {
    ...
    const { doctorId, appointmentDate, appointmentTime, consultationFee, serviceCharge, type } = req.body;
    ...
    try {
        ...
        if (!canBookAppointment(appointmentDate, appointmentTime, doctor)) {
            return res.status(400).json({ msg: 'Please book before appointment time', success: false });
        }

        // 4️⃣ Prevent double booking for the same doctor at the same time
        const existingBooking = await Booking.findOne({ doctorId, appointmentDate, appointmentTime });
        if (existingBooking) {
            return res.status(400).json({ success: false, msg: `Doctor is already booked at ${TimeFormate(appointmentTime)} !` });
        }
        let query = {
            doctorId: doctorId,
        ...
        if (checkPatientAlready) {
            return res.status(400).json({ success: false, msg: `Patient is already booked at the same date ${appointmentDate}!` });
        }
        ...
        const newBooking = new Booking({
            patientId: mongoose.Types.ObjectId.isValid(patientId) ? patientId : userId, // If patientId is not provided, use userId
            userId,
            doctorId,
            appointmentDate,
            appointmentTime,
        ...
        await bookingNotification(doctorId, mongoose.Types.ObjectId.isValid(patientId) ? patientId : userId, appointmentDate, appointmentTime)
        await newBooking.save();

        return res.status(201).json({ success: true, msg: "Booking confirmed successfully!", result: newBooking });
...
exports.confirmBooking = async (req, res) => {
    ...
    try {
        ...
        if (booking.bookingStatus === 'confirmed') {
            return res.status(400).json({ success: false, msg: "Booking is already confirmed!" });
        }

        booking.bookingStatus = 'confirmed';
        await booking.save();

        return res.status(200).json({ success: true, msg: "Booking confirmed successfully!", booking });
...
exports.cancelBooking = async (req, res) => {
    ...
    try {
        ...
        return res.status(200).json({ success: true, msg: "Booking cancelled successfully!", booking });
...
exports.addBookingByReceptionist = async (req, res) => {
    ...
    const { doctorId, appointmentDate, patientDetails, consultationFee = 0, serviceCharge = 0 } = req.body;
    ...
    try {
        ...
        if (existingPatient) {
            ...
            patient = new Patient({
                ...
                emergencyNumber: patientDetails?.emergencyNumber ? patientDetails?.emergencyNumber : patientDetails?.mobile,
                address: patientDetails?.address,
        ...
        if (!doctor.startTime || !doctor.endTime) {
            return res.status(400).json({ success: false, msg: "Doctor's working hours are not set." });
        }
        ...
        if (!availableSlot) {
            return res.status(400).json({ success: false, msg: "No available slots for the selected date." });
        }

        // 9️⃣ Prevent double booking for the same patient on the same day
        const existingBooking = await Booking.findOne({ doctorId, appointmentDate, patientId: patient._id });

        if (existingBooking) {
            return res.status(400).json({ success: false, msg: `Patient already booked on ${appointmentDate}.` });
        }

        // 🔟 Calculate total amount
        const totalAmount = Number(consultationFee) + serviceCharge ? Number(serviceCharge) : 0;
