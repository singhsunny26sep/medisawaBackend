...
exports.getAllWithPagination = async (req, res) => {
    ...
    try {
        const result = await Offer.find().sort({ createdAt: -1 }).skip(skip).limit(limit);
...
exports.addOffer = async (req, res) => {
    ...
    try {
        ...
        if (checkOffer) {
            return res.status(400).json({ success: false, msg: "Offer already exists!" })
        }
        ...
        if (result) {
            return res.status(200).json({ success: true, msg: "Offer added successfully!", offer: result })
        }
    } catch (error) {
...
exports.updateOffer = async (req, res) => {
    ...
    try {
        ...
        if (result) {
            return res.status(200).json({ msg: "Offer updated successfully!", data: result, success: true })
        }
        return res.status(400).json({ msg: 'Failed to update offer!', success: false })
    } catch (error) {
...
exports.deleteOffer = async (req, res) => {
    ...
    try {
        ...
        if (result) {
            return res.status(200).json({ msg: "Offer deleted successfully!", data: result, success: true })
        }
        return res.status(400).json({ msg: 'Failed to delete offer!', success: false })
    } catch (error) {
