...
exports.getAllWithPagination = async (req, res) => {
    ...
    try {
        const result = await Brand.find().sort({ createdAt: -1 }).skip(skip).limit(limit);
...
exports.addBrand = async (req, res) => {
    ...
    try {
        ...
        if (result) {
            return res.status(200).json({ success: true, msg: "Brand added successfully!", data: result })
        }
        return res.status(400).json({ success: false, msg: "Failed to add Brand!" })
    } catch (error) {
...
exports.updateBrand = async (req, res) => {
    ...
    try {
        ...
        if (result) {
            return res.status(200).json({ success: true, msg: "Brand updated successfully!", data: result })
        }
        return res.status(400).json({ success: false, msg: "Failed to update Brand!" })
...
exports.deleteBrand = async (req, res) => {
    ...
    try {
        ...
        if (result) {
            return res.status(200).json({ success: true, msg: "Brand deleted successfully!", data: result })
        }
        return res.status(400).json({ success: false, msg: "Failed to delete Brand!" })
