...
exports.contactPagination = async (req, res) => {
    ...
    try {
        const result = await Contact.find().limit(limit).skip(skip).sort({ createdAt: -1 })
...
exports.addCotactForm = async (req, res) => {
    ...
    try {
        const result = await Contact.create({ name, email, mobile, message, reason, subject })
        return res.status(200).json({ success: true, msg: 'Contact form added successfully!', result })
    } catch (error) {
...
exports.updateContact = async (req, res) => {
    ...
    try {
        ...
        if (result) {
            return res.status(200).json({ success: true, msg: "Contact updated successfully!", result })
        }
        return res.status(400).json({ success: false, msg: "Failed to update contact!" })
    } catch (error) {
...
exports.deleteContact = async (req, res) => {
    ...
    try {
        ...
        if (result) {
            return res.status(200).json({ success: true, msg: "Contact deleted successfully!", result })
        }
        return res.status(400).json({ success: false, msg: "Failed to delete contact!" })
    } catch (error) {
