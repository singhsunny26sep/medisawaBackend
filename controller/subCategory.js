...
exports.getAllSubCategory = async (req, res) => {
    ...
    try {
        ...
        const result = await SubCategory.find().sort({ createdAt: -1 }).populate("category")
        if (result) {
            return res.status(200).json({ success: true, result })
        }
...
exports.getAllWithPaginationSubCategory = async (req, res) => {
    ...
    try {
        const result = await SubCategory.find().sort({ createdAt: -1 }).skip(skip).limit(limit).populate("category")
...
exports.getSubCategoryByCategory = async (req, res) => {
    ...
    try {
        ...
        const result = await SubCategory.find({ category: id }).sort({ createdAt: -1 }).populate("category")
...
exports.addSubCategory = async (req, res) => {
    ...
    try {
        ...
        const subcategory = new SubCategory({ title, description, category: category })
        if (image) {
