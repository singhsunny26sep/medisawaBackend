...
exports.addPlan = async (req, res) => {
    ...
    try {
        ...
        if (planCost < 0) {
            return res.status(400).json({ success: false, msg: "Plan cost cannot be negative!" })
        }
        if (discount) {
            if (discount < 0) {
        ...
        if (checkPlan) {
            return res.status(400).json({ success: false, msg: "Plan already exists!" })
        }
        ...
        if (result) {
            return res.status(201).json({ success: true, msg: "Plan added successfully!", data: result })
        }
...
exports.updatePlan = async (req, res) => {
    ...
    try {
        ...
        if (checkByName) {
            return res.status(400).json({ success: false, msg: `Plan with the same name ${name} already exists!` })
        }
        ...
        if (result) {
            return res.status(200).json({ success: true, msg: "Plan updated successfully!", data: result })
        }
        return res.status(400).json({ success: false, msg: "Failed to update plan!" })
    } catch (error) {
...
exports.deletePlan = async (req, res) => {
    ...
    try {
        ...
        if (result) {
            return res.status(200).json({ success: true, msg: `Plan deleted successfully!` })
        }
        return res.status(404).json({ success: false, msg: "Plan not found!" })
    } catch (error) {
