...
exports.getNearbyClinics = async (req, res) => {
  ...
  if (!lat || !lng) {
    return res
      .status(400)
      .json({ success: false, msg: "Latitude and Longitude are required." });
  }
...
exports.addClinic = async (req, res) => {
  ...
  try {
    ...
    if (checkClinic) {
      return res
        .status(400)
        .json({ success: false, msg: "Clinic already exists!" });
    }
    let imageUrl = "";
    if (image) {
    ...
    if (!newClinic) {
      return res
        .status(400)
        .json({ success: false, msg: "Failed to create clinic!" });
    }
...
exports.deleteClinic = async (req, res) => {
  ...
  try {
    ...
    if (result) {
      return res
        .status(200)
        .json({ success: true, msg: "Clinic deleted successfully", result });
    }
    return res
      .status(400)
      .json({ success: false, msg: "Failed to delete clinic!" });
  } catch (error) {
