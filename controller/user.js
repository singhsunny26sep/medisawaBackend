...
exports.registorUser = async (req, res) => {
    ...
    try {
        ...
        const patient = await Patient.create({ userId: user._id, name, email, mobile, password: hashedPass, role, address })
...
exports.userProfile = async (req, res) => {
    ...
    try {
        const result = await User.findById(id).populate("patientId").populate({ path: "doctorId", populate: [{ path: "specialization", select: "name" }, { path: "department", select: "name" }] }).populate('clinic')
        if (result) {
            return res.status(200).json({ success: true, result })
        }
