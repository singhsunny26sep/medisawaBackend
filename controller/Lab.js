...
exports.addTest = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json({ success: true, msg: "Test added successfully!", data: result });
  } catch (error) {
...
exports.updateTest = async (req, res) => {
  ...
  try {
    ...
    if (checkUser.role != "admin") {
      if (checkTest.userId.toString() != userId) {
        return res.status(403).json({
          success: false,
          msg: "You are not authorized to update this test!",
        });
      }
    }
    ...
    return res
      .status(200)
      .json({ success: true, msg: "Test updated successfully!", data: result });
  } catch (error) {
...
exports.deleteTest = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json({ success: true, msg: "Test deleted successfully!" });
  } catch (error) {
...
exports.getTestLabPatient = async (req, res) => {
  const userId = req.payload?._id;
  try {
    const result = await LabTest.find({ labId: userId, paid: true })
      .sort({ createdAt: -1 })
      .populate(
        "patientId",
      ...
      .populate("test", "-__v")
      .populate("labId", "-password -patientId -__v -createdAt -updatedAt")
...
exports.updateReport = async (req, res) => {
  ...
  try {
    ...
    if (
      ...
      (checkUser.role != "receptionist" &&
        checkReport.labUser.toString() != userId)
    ) {
      return res.status(403).json({
        success: false,
...
exports.deleteReport = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json({ success: true, msg: "Report deleted successfully!" });
  } catch (error) {
    console.error("Error on updateReport:", error);
    return res.status(500).json({
      success: false,
...
exports.mostTakenTest = async (req, res) => {
  try {
    ...
    if (!result.length) {
      ...
      return res.status(200).json({
        msg: "Most taken test this month",
        success: true,
...
exports.getByBookingId = async (req, res) => {
  const id = req.params?.id;
  try {
    const result = await LabTest.find({ appointmentId: id })
      .sort({ createdAt: -1 })
      .populate(
        "patientId",
        "-allergies -department -__v -createdAt -updatedAt"
    ...
    return res
      .status(404)
      .json({ success: false, msg: "No Lab Tests found for this booking!" });
  } catch (error) {
...
exports.getByPatientId = async (req, res) => {
  const id = req.params?.id;
  try {
    const result = await LabTest.find({ patientId: id })
      .sort({ createdAt: -1 })
      .populate(
        "patientId",
        "-allergies -department -__v -createdAt -updatedAt"
...
exports.getByDoctorId = async (req, res) => {
  const id = req.params?.id;
  try {
    const result = await LabTest.find({ doctorId: id })
      .sort({ createdAt: -1 })
      .populate(
        "patientId",
        "-allergies -department -__v -createdAt -updatedAt"
...
exports.getByLabId = async (req, res) => {
  ...
  try {
    ...
    const result = await LabTest.find(filetr)
      .sort({ createdAt: -1 })
      .populate(
        "patientId",
        "-allergies -department -__v -createdAt -updatedAt"
...
exports.bookTest = async (req, res) => {
  ...
  try {
    ...
    const newLabTest = new LabTest({
      patientId: patientId ? patientId : checkUser?.patientId[0],
