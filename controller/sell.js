...
exports.sellesSingle = async (req, res) => {
    ...
    try {
        ...
        if (checkSize.quntity < quntity) {
            return res.status(400).json({ success: false, msg: `Stock not enough! you can buy ${checkSize?.quntity}` });
        }
        let amount = 0;
        if (checkSize?.discount > 0) {
            if (checkSize.discountType === 'percentage') {
                amount = (quntity * checkSize?.price) - ((quntity * checkSize?.price) * checkSize?.discount / 100);
            } else {
        ...
        const result = await Sell.create({ userId, sizeId, medicineId: checkMedicine._id, quantity: quntity, priceAtSale, amount });
        if (result) {
...
exports.sellMultiple = async (req, res) => {
    ...
    try {
        for (const item of items) {
            ...
            const checkMedicine = await Medicine.findById(checkSize.medicine).session(session);
            if (!checkMedicine) throw new Error('Medicine not found');

            if (checkSize.quntity < quantity) {
                throw new Error(`Not enough stock for ${checkMedicine.title} ${checkSize.size}. Available: ${checkSize.quntity}`);
            }
            ...
            if (checkSize.discount > 0) {
                if (checkSize.discountType === 'percentage') {
                    amount = (quantity * checkSize.price) - ((quantity * checkSize.price) * checkSize.discount / 100);
                } else {
            ...
            // Create Sell record
            const sell = await Sell.create([{ userId, sizeId, medicineId: checkMedicine._id, quantity, priceAtSale, amount }], { session });
            sells.push(sell[0]);
        }
...
exports.getSingleUsersSellesHistory = async (req, res) => {
    const userId = req.payload?._id;
    try {
        const result = await Sell.find({ userId }).populate("sizeId", "-__v").populate("medicineId", "-__v");
        if (result) {
            return res.status(200).json({ success: true, result });
        }
        return res.status(400).json({ msg: 'Something went wrong!', success: false });
